...
/* ════════════════════════════════════════════════════════════════════════
   ...
   zamykany HANGAREM, nie zegarem; KAŻDA flota pod uderzeniem ma własny ratunek
   (do v3.74 była „jedna ucieczka na parę" — to ona kosztowała 152 mln statków
   10.09); nic nie leci NA atakowane ciało; nieznany markup → zrzut do logu,
   nie zgadywanie; kanał alarmowy = tylko obrona (ekonomia rodzajem „EKO").
   ════════════════════════════════════════════════════════════════════════ */
(function () {
  "use strict";
  ...
  const Notifier = {
    ...
    // ucieczka spod ostrzału — „🛟 Flota ewakuowana". Właściciel uczył się w ten sposób
    // ignorować kanał, którym przychodzi jedyny sygnał o utracie floty.
    // v3.77.0 (HANDOFF-2026-09-11, pkt 4): rodzaj „EKO" wydzielony z „BŁĄD" — ta sama
    // lekcja co przy „FS" wyżej. Zatrzymana ekspedycja, przerwana fala mineru czy zbiórka
    // złomu to KŁOPOT EKONOMII: nie ma godziny, o której musi obudzić, i nie wymaga żadnej
    // reakcji w minutę. Szły jednak tytułem „⚠️ Obrona: BŁĄD (Genesis)", z priorytetem
    // `high`, czyli tym samym kanałem i tą samą wagą co jedyny sygnał o realnym ostrzale —
    // a kanał, na którym większość alarmów okazuje się nieszkodliwa, przestaje się czytać.
    // Dzwonek zostaje przy obronie; ekonomia dostaje własny tytuł, niski priorytet i rzadszy
    // dławik (30 min — te same kłopoty powtarzają się falami).
    THROTTLE: { ATAK: 5 * 60e3, RATUNEK: 2 * 60e3, POWRÓT: 5 * 60e3, BŁĄD: 5 * 60e3, FS: 10 * 60e3, EKO: 30 * 60e3 },
    // v3.77.0 (HANDOFF-2026-09-11, pkt 1 — P0) — CO SIĘ PSUŁO: temat ntfy był LOSOWANY
    // przy pierwszym uruchomieniu i zapamiętywany w schowku Tampermonkeya, a schowek jest
    // PER PRZEGLĄDARKA/URZĄDZENIE. Po przesiadce z Firefoksa na Chrome (09/10.09) bot
    // wylosował sobie nowy temat („ogamex3-6wjps7yzsw7t"), telefon dalej słuchał starego —
    // przez dobę ŻADEN alarm o ataku nie miał dokąd dojść, a panel przy tym pokazywał
    // „push ON" i test pusha „przechodził" (szedł w pustkę, którą nikt nie subskrybuje).
    // Losowy temat ma sens dla wielu niezależnych instalacji; tu jest JEDEN właściciel
    // i JEDEN telefon, więc kanał alarmowy musi być WŁASNOŚCIĄ KODU — ten sam wzorzec co
    // `pinCodeOwned` dla wykluczeń ekspedycji. Temat jest wspólny ze strażnikiem
    // (`watchdog/ogx-watchdog.py`, `watchdog/ogx-heal.sh`), więc telefon ma JEDNĄ
    ...
    topic() {
      ...
      if (t !== this.TOPIC) {
        Store.set("ntfy_topic", this.TOPIC);   // trzymamy w schowku dla narzędzi czytających stan z dysku
        if (t) log(`[PUSH] temat ntfy z tej przeglądarki (${t}) był inny niż kanał telefonu — od teraz ${this.TOPIC}.`, "warn");
      }
      return this.TOPIC;
    },
  ...
  const DEFAULTS = {
    ...
    // v3.68.10 (audyt 04.09, obrona-fs#2 P1): `slotReserve` — ile slotów floty MUSI zostać
    // wolnych, żeby Fleet Save w ogóle wystartował. Reguła FS siedzi w pętli po WSZYSTKICH
    // parach i do 3.68.9 nie miała żadnego odpowiednika rezerwy slotów (mają ją expo i
  ...
  // v3.78.0 (decyzja właściciela 11.09, DOKTRYNA-DESTROY-2026-09-11): ucieczka ma lecieć
  // „możliwie najmniejszą prędkością, może być 3%" — flota ma WISIEĆ w locie w chwili
  ...
  const Situation = {
    ...
    async refresh() {
      ...
      // v3.68.10 (audyt 04.09, obrona-fs#3 P1) — CO SIĘ PSUŁO: zniszczenie księżyca
      // zapalało `s.moonLost`, ale NIE kasowało wpisu hangaru "klucz|moon". Wpis nie ma
      // własnego terminu ważności (konsumenci akceptują go do 48 h), więc przez dwie doby
      // udawał, że flota stoi na ciele, którego nie ma — łamiąc regułę „żaden wpis stanu
      // nie może być wieczny" dokładnie tam, gdzie boli najbardziej. Kasujemy PO scaleniu
      // z drugą kartą: gdyby stało wyżej, blok scalający (`!mine` → bierz z cur) wskrzesiłby
      // ducha w tym samym przebiegu. Odwrotne przejście (Moon odbudował księżyc) niczego
      // nie kasuje — `hasMoon` jest wtedy znowu prawdziwe.
      for (const hk of Object.keys(s.hangars || {})) {
        ...
        delete s.hangars[hk];
        log(`[KSIĘŻYC] [${hkey}] nie ma księżyca, a w stanie leżał jego hangar sprzed ${Math.round((now - (h.at || 0)) / 60000)} min (${(h.total || 0).toLocaleString("pl-PL")} szt.) — kasuję. Bot nie będzie udawał, że flota stoi na nieistniejącym ciele.`, "warn");
      }
      // v3.68.10 (audyt 04.09, obrona-fs#4 P1): zmierzone czasy lotu FS (patrz Fly.tick,
    ...
    fleetAt(s, k, now = Date.now()) {
      // v3.68.10 (audyt 04.09, obrona-fs#3 P1) — CO SIĘ PSUŁO: DUCH KSIĘŻYCA. Po
      // zniszczeniu księżyca (realny incydent 26.08 i noc 03/04.09) wpis hangaru
      // "klucz|moon" żył dalej — do 48 h — i udawał miejsce postoju floty na ciele,
      // którego już NIE MA. Skutki były trzy, wszystkie ciche: przy ataku w planetę
      // decide() mówił „flota na moon — bezpieczna strona" i nie robił NIC; Fleet Save
      // startował z nieistniejącego księżyca i kończył głośnym abortem trzy razy na
      // godzinę; samokontrola meldowała „obrona gotowa" nad flotą, której nie widziała.
      // Para bez księżyca nie ma jak trzymać floty na księżycu — ten hangar nie istnieje.
  ...
  function decide(s, cfg, now) {
    ...
    // księżycu, więc gdy ucieka falami wracającymi z ekspedycji co kilka minut, musi brać
    // zawsze 1 sztukę na slot floty"). Gwiazda Śmierci jest najwolniejszym statkiem, więc
    // JEDNA sztuka spowalnia CAŁY lot — a wolny lot to ten, który zdąży się zawrócić
    // (doktryna DESTROY, krok 4: zawrócone „stacjonuj" jest niewidoczne na falandze).
    // SUFIT, nie wykluczenie: ucieczka dalej zabiera cały hangar, tylko GS jest racjonowana
    // po jednej na lot. Gdyby pierwsza fala zabrała wszystkie 30, każda następna — a fale
    // wracają z ekspedycji co kilka minut — leciałaby SZYBKO i wylądowała, zanim padnie zawrót.
    // Sufit jest własnością KODU (nie ma go w CFG): to reguła fizyki gry, nie preferencja.
    const CAP_RATUNKU = { DEATH_STAR: 1 };
    ...
    // wykonawcza oddaje go PIERWSZEJ akcji lotu. Dopóki lot RUTYNOWY („dom = księżyc",
    // „powrót po ratunku") w ogóle POWSTAWAŁ w przebiegu z atakiem, potrafił ten slot
    // zabrać — najdotkliwiej w oknie potwierdzania zagrożenia (confirmMs), kiedy akcji
    // ratunku jeszcze NIE MA, więc nie ratuje ani sortowanie, ani strażnik w pętli.
    // E2E z audytu: bot wysyłał „powrót po ratunku" (stempel `rescues` żyje 6 h po KAŻDEJ
    // ucieczce, czyli dokładnie w noc drugiej fali), a ratunek atakowanej pary czekał na
    // domknięcie tamtej misji. Zwożenie floty na własny księżyc nie ma ŻADNEJ wartości
    // obronnej, więc przy ataku gdziekolwiek na koncie po prostu nie powstaje.
    // (Fleet Save zostaje w `actions` — jego wybór celu i alarmy o złym celu muszą działać
    // także w trakcie ataku na inną kolonię; o slot dba strażnik w pętli wykonawczej,
    // który zna twarde ograniczenie „jeden lot na przebieg".)
    ...
    // v3.68.10 (audyt 04.09, obrona-fs#3 P1): to samo, co w Situation.fleetAt — hangar
    // księżyca pary, która księżyca NIE MA, opisuje ciało nieistniejące. Bez tej bramki
    // „flota na obu ciałach" liczyła ducha jako drugie miejsce postoju i alarmy mówiły
    // operatorowi, że coś stoi tam, gdzie stać nie może.
    const fleetsAt = (k) => ["moon", "planet"]
      .map(b => ({ body: b, h: (b === "moon" && pairs[k] && pairs[k].hasMoon === false) ? null : (s.hangars || {})[`${k}|${b}`] }))
      .filter(x => x.h && (x.h.total || 0) > 0 && now - (x.h.at || 0) < 48 * 3600e3)
      .map(x => ({ body: x.body, total: x.h.total, at: x.h.at, ships: x.h.ships || [] }));
    ...
    for (const k of Object.keys(pairs)) {
      ...
      if (!th.length) {
        ...
        if ((!f || fsDosylowy) && fleet && cfg.fs && cfg.fs.enabled && fleet.total > 0 && fleet.body === "moon") {
          ...
          // v3.68.10 (audyt 04.09, obrona-fs#4 P1) — CO SIĘ PSUŁO: PĘTLA „LOT ZA KRÓTKI".
          // Fly wymaga, żeby lot trwał co najmniej POŁOWĘ czasu do godziny powrotu (inaczej
          // flota doleci i WYLĄDUJE zamiast wisieć w powietrzu) — czyli start ma sens tylko
          // w oknie [homeAt − 2·czas lotu, homeAt]. decide() o czasie lotu nie wiedziała nic
          // i wystawiała akcję FS przy KAŻDYM przebiegu, więc zaraz po porannym powrocie
          // (half ≈ 12 h) bot przez resztę doby, co godzinę, trzykrotnie: przejmował
          // operatorowi aktywne ciało, wchodził na /fleet, wypełniał cały formularz, czytał
          // czas lotu i przerywał — ~72 porzucone misje i ~20 pushy „⚠️ BŁĄD" na dobę, przy
          // zielonym pasku stanu. Teraz zmierzony czas lotu wraca do stanu (`s.fsMeasured`,
          ...
          if (dest) {
            ...
            const znany = !!short && short.flightMs > 0 && (short.speedPct || 0) === fsSpeed && now - (short.at || 0) < 24 * 3600e3;
      ...
      // v3.79.0 (decyzja właściciela 11.09, po tym jak E2E sc. 63 pokazało OSIEM lotów po
      // jednej sztuce): Gwiazdy Śmierci stojące w domu są REZERWĄ — każda kolejna fala
      // wracająca z ekspedycji zabiera z niej jedną sztukę, żeby lecieć wolno i dać się
      ...
      // klucz identyczny — a `sort` jest stabilny, czyli o tym, którą kolonię bot uratuje
      // swoim JEDYNYM slotem lotu, decydowała kolejność par na pasku planet. Skoordynowany
      // nalot (dolot 600 s na pustawy księżyc, 80 s na główny hangar) ratował więc kolonię,
      // której nic nie groziło. Pilność MUSI powstać tutaj, w czystej funkcji — warstwa
      // wykonawcza nie zna ani zegara zagrożenia, ani wielkości ratowanego hangaru.
      ...
      if ((other === "planet" || pairs[k].hasMoon) && !bodies.has(other) && !bodies.has("unknown")) {
        actions.push({ kind: "fly", fromKey: k, fromBody: fleet.body, toKey: k, toBody: other,
          ...
          speed: nbBlocked ? cfg.airSpeedPct : 100, recall: false, rescue: true, capTypes: CAP_RATUNKU, etaMs, saveTotal });
        continue;
      }
    ...
    // WSZYSTKICH parach i — jako jedyny moduł wysyłający floty — nie miała ani ograniczenia
    // do jednego ciała startowego (`launchFrom` w expo/aster), ani rezerwy slotów
    // (`slotReserve`, tamże). Konto z pięcioma księżycami z flotą dostawało PIĘĆ osobnych
    // akcji FS na ten sam cel; każdy taki lot zjada slot floty i zaślepia swoją parę
    // (`inFlightFrom`). Gdy sloty się kończyły, gra po prostu odmawiała wysyłki RATUNKU —
    // a pasek stanu przez cały ten czas świecił zielonym „FS w drodze". Teraz: z przebiegu
    ...
    if (fsCands.length) {
      ...
      const fsSlots = (s.slots && s.slots.fleet && now - (s.slots.at || 0) < 30 * 60e3) ? s.slots.fleet : null;
      ...
      // Odczyt slotów jest robiony TYLKO na stronie floty, więc bywa nieznany. Wtedy nie
      // zgadujemy liczb: pozwalamy na JEDEN lot FS w powietrzu naraz — tyle wiemy na pewno.
      ...
      if (wolne > 0) {
        ...
        alerts.push({ key: best.fromKey, level: "warn", throttleMs: 30 * 60e3, msg: fsSlots
          ? `FS: sloty floty ${fsSlots.used}/${fsSlots.total}, rezerwa ${reserve} — nie wysyłam Fleet Save, bo ostatnie wolne sloty należą do ratunku`
          : `FS: jeden lot Fleet Save już wisi w powietrzu, a liczby slotów floty nie znam (odczyt ze strony floty starszy niż 30 min) — kolejnego nie wysyłam, żeby zostało czym uciekać` });
      }
    }
    ...
    if (s.barExcess && s.barExcess.active) {
      ...
      if (withFleet.length) {
        ...
        if (dest) actions.push({ kind: "fly", fromKey: t.k, fromBody: t.f.body, toKey: dest.key, toBody: dest.body, why: "ŚLEPY ALARM (pasek widzi atak, listy brak)", speed: cfg.airSpeedPct, recall: true, air: true, blind: true, capTypes: CAP_RATUNKU, saveTotal: t.f.total, recallAt: now + 10 * 60e3 });
  ...
  const Human = {
    ...
    // v3.68.10 (audyt 04.09, obrona-fs#5 P2) — CO SIĘ PSUŁO: pauza „flota jest na Fleet
    // Save" gasiła CAŁĄ ekonomię, w tym dwa moduły, które z flotą na FS nie mają nic
    // wspólnego — a których statki FS SAM zostawia w domu „bo pracują". Odbudowa księżyca
    // (Moon) nie wysyła ani jednego statku, a zniszczenie księżyca w środku lotu FS to
    // ulubiony moment napastnika: bez tej poprawki księżyc czekał na odbudowę do powrotu
    ...
    economyAllowed(s, who = "") {
      ...
      // v3.111.1 (owner 23:29: „nie wysyłał flot na ekspy" — 60 s ciszy po KAŻDYM kliknięciu
      // przy aktywnej grze = seria stała 3/14): liczy się tylko ZMIANA STRONY, pauza 20 s,
      // a całe ustępowanie ma sufit 2 min — potem fala leci, nawet gdy dalej klikasz.
      if (this.yielding()) {
        const y = Store.get("eco_yield", null);
    ...
    learn() {
      ...
      for (const item of document.querySelectorAll(".galaxy-item")) {
        ...
        if (!mission) {
          ...
          if (n >= 3 && !Once.said("expo_link_fail", 6 * 3600e3)) Journal.add("EKO", `Nie umiem odczytać id misji ekspedycji z wiersza 16 galaktyki (${n} prób) — ekspedycje STOJĄ. Zrzut markupu jest w logu.`);
          return;
        }
  ...
  const Expo = {
    ...
    noteStall(p, now) {
      ...
      Journal.add("EKO", `Ekspedycje stoją od ${Math.round((now - since) / 60e3)} min: ${p.skip}. To priorytet nr 2 — sprawdź, gdzie stoi flota.`);
    },
  ...
  const Debris = {
    ...
    async tick(s) {
      ...
      // v3.68.10 (audyt 04.09, obrona-fs#5): recyklery są wykluczone z lotu Fleet Save
      // DOKŁADNIE po to, żeby w tym czasie pracowały („miner/recykler w trakcie pracy
      // zostaje w domu") — pauzowanie ich na czas FS było wewnętrznie sprzeczne i zostawiało
      // złom po bitwie na polu, choć zbieracze stały bezczynnie w hangarze.
  ...
  const Fly = {
    ...
    start(a) {
      ...
      // v3.68.10 (testy-architektura#1): Fleet Save ma własny rodzaj wpisu — rutynowy lot
      // nocny nie może iść tym samym kanałem push co ucieczka spod ostrzału.
    ...
    abort(why, opts = {}) {
      ...
      // v3.77.0 (HANDOFF-2026-09-11, pkt 4): przerwana fala ekspedycji/mineru/złomu to
      // kłopot ekonomii, nie awaria obrony — ten sam podział rodzajów co w `start()` wyżej.
    ...
    confirmed(m, info = {}) {
      ...
      // i nie-ekonomia" obejmował od v3.68.0 także FLEET SAVE (`m.home` undefined,
      // `m.kind` = "fly"). Stempel czyta decide() jako `backFromRescue` i przez 6 h OMIJA
      // wyłączone `homeToMoon`, czyli decyzję ownera z 30.08 („przenosić flotę ma tylko
      // podczas ataku"): po rutynowym powrocie FS bot sam zwoził na księżyc wszystko, co
    ...
    async tick() {
      ...
      // „otwarte ręcznie" + świeży klik zapisuje `eco_yield`) = operator zabrał kartę. Dawniej pierwszy
    ...
    async form(m) {
      ...
      // v3.79.0: `capTypes` = SUFIT sztuk danego typu w tym locie (dziś: jedna Gwiazda Śmierci
      // w ucieczce — spowalnia lot, a reszta GS zostaje na następne fale). Inaczej niż
      // `excludeTypes`, który zeruje typ w całości: tu bierzemy min(ile mam, sufit).
      const cap = new Map(Object.entries(m.capTypes || {}).map(([t, n]) => [String(t).toUpperCase(), Math.max(0, parseInt(n, 10) || 0)]));
      // v3.79.0 (złapane przez E2E sc. 63, potem decyzja właściciela): sufit ma sens TYLKO
      // jako dodatek do ratowanej floty. Gdy w hangarze nie ma już nic poza typem z sufitem,
      // „zabierz jedną" znaczy „wyślij lot z jedną GS" — a decide() przy następnym przebiegu
      // widzi resztę na atakowanym ciele i wystawia kolejny taki lot. W teście bot wysłał
      // OSIEM lotów po jednej Gwieździe Śmierci, paląc slot floty na każdą z nich.
      ...
      // v3.103.0 (log z żywej gry 21.09 11:11): plan „cały hangar 2 802 mln ≤ udział fali", a między planem
      // a formularzem wylądowała KOLEJNA fala — poleciało 5 600 mln jednym lotem. Jeden lot zajął miejsce
      // dwóch, więc slot stał pusty ponad godzinę (w domu została resztka 37 mln, której bot słusznie nie
      // wysyła). To samo zlepianie, które v3.101.0 usunęła z planu, wracało przez formularz. Udział fali
      // liczymy więc jeszcze raz na tym, co formularz pokazuje TERAZ: gdy stoi tu co najmniej 1,5 udziału,
      // wylądowała druga fala — leci jeden udział, reszta pójdzie następnym lotem w wolny slot.
      ...
      if (cap.size && !want) {
        ...
        if (!jestCosPoza) {
          ...
          return this.abort("w hangarze sama rezerwa spowalniająca — zostaje w domu", { quiet: true });
        }
      }
      for (const el of els) {
      ...
      if (m.speed && m.speed !== 100) {
        // v3.78.0 (DOKTRYNA-DESTROY-2026-09-11, pkt 4.1) — CO SIĘ PSUŁO: bot szukał w rzędzie
        // prędkości elementu o tekście DOKŁADNIE równym żądanej liczbie. Gdy fork takiej opcji
        // nie ma (właściciel prosi o „możliwie najmniejszą, może być 3%", a fork może dawać
        // same dziesiątki), nie klikał NICZEGO i lot szedł z domyślną prędkością — czyli 100%.
        // To nie jest drobiazg: cały sens ucieczki przed DESTROY polega na tym, żeby flota
        ...
        for (const h of [...document.querySelectorAll("a, span, button, div, td, li")].filter(e => txt(e) === "100" && e.offsetParent !== null && !e.closest("#ogx3-panel"))) {
          ...
          dostepne = kids.map(k => ({ el: k, pct: /^\d{1,3}$/.test(txt(k)) ? parseInt(txt(k), 10) : null }))
            .filter(x => x.pct !== null && x.pct > 0 && x.pct <= 100);
          const nieSzybsze = dostepne.filter(x => x.pct <= m.speed);
          const wybor = nieSzybsze.length
            ? nieSzybsze.reduce((a, b) => (b.pct > a.pct ? b : a))
            : (dostepne.length ? dostepne.reduce((a, b) => (b.pct < a.pct ? b : a)) : null);
          if (wybor) { wybor.el.click(); ok = true; wybrana = wybor.pct; m.speedUsed = wybrana; }
          break;
        }
        if (ok && wybrana !== m.speed) log(`[LOT] prędkości ${m.speed}% nie ma na liście forka (są: ${dostepne.map(x => x.pct).join(", ")}) — lecę najbliższą NIE SZYBSZĄ: ${wybrana}%.`, "warn");
        else if (ok) log(`[LOT] lista prędkości forka: ${dostepne.map(x => x.pct).join(", ")}.`, "info");
      ...
      if (czasLotu > 0) {
        ...
        if (m.fs && m.recallAt) {
          ...
          // v3.68.10 (audyt 04.09, obrona-fs#4 P1) — CO SIĘ PSUŁO: czas lotu znała WYŁĄCZNIE
          // ta funkcja, przez sekundę, przy otwartym formularzu. decide() nie wiedziała nic,
          // więc wystawiała akcję FS przy każdym przebiegu — także wtedy, gdy odmowa była
          // PEWNA (poza oknem [homeAt − 2·czas lotu, homeAt]). Bot spalał na to 3 misje na
          // godzinę, całą dobę: przejęcie ciała operatorowi, wejście na /fleet, cały
          // formularz, abort — plus push „BŁĄD" co godzinę i zielony pasek stanu. Teraz
          // POMIAR WRACA DO STANU (`s.fsMeasured`) i decide() liczy z niego to samo okno.
          // Zapisujemy w OBU wynikach pomiaru — także po udanym starcie — żeby wiedza się
          // nie zestarzała i żeby jutro nie trzeba było jej kupować kolejną porzuconą misją.
          // Klucz = trasa, wpis niesie prędkość (inna prędkość = inny czas lotu) i stempel;
          // kasowany po dobie w Situation.refresh (żaden wpis stanu nie jest wieczny).
          const zapamietajLot = () => { try { const sS = Situation.load(); sS.fsMeasured = sS.fsMeasured || {}; sS.fsMeasured[`${m.fromKey}>${m.toKey}`] = { flightMs: m.flightMs, speedPct: m.speed || 0, at: Date.now() }; Situation.save(sS); } catch {} };
          zapamietajLot();
          if (m.flightMs < half) {
            const mins = (ms) => Math.round(ms / 60e3);
            if (!Once.said(`fsshort|${m.fromKey}`, 60 * 60e3)) Journal.add("BŁĄD", `Fleet Save odwołany: lot [${m.fromKey}]→[${m.toKey}] trwa ${mins(m.flightMs)} min, a do powrotu o ${hh(homeAt)} zostało ${mins(homeAt - t0)} min. Flota doleciałaby i WYLĄDOWAŁA zamiast wisieć w powietrzu. Startuję dopiero o ${hh(homeAt - 2 * m.flightMs)}; jeśli ma być wcześniej — zmniejsz prędkość FS albo ustaw dalszy cel.`);
  ...
  function emptySourceHangar(fromKey, fromBody, why, keepTypes, capTypes) {
    try {
      ...
      // następna fala nie miałaby czym zwolnić lotu, a bot „nie widziałby" floty, która tam stoi.
      ...
      const left = (keep.size || cap.size) ? (h.ships || []).map(x => {
        ...
        if (cap.has(t)) { const zostalo = Math.max(0, (x.qty || 0) - cap.get(t)); return zostalo > 0 ? { ...x, qty: zostalo } : null; }
        return null;
      }).filter(Boolean) : [];
  ...
  async function defenceTick() {
    ...
    try {
      ...
      // v3.78.0 (DOKTRYNA-DESTROY-2026-09-11, pkt 4.2): DESTROY dzieli wór z ATTACK i trasa
      // ratunku ma zostać ta sama (decyzja właściciela) — ale NIE komunikat. „ATTACK" znaczy
      // „stracisz flotę, jeśli nie ucieknie"; „DESTROY" znaczy „stracisz KSIĘŻYC, nawet jeśli
      // flota ucieknie", a to zmienia to, czego właściciel ma się spodziewać po przebudzeniu:
      // wróci do pary bez księżyca, z flotą w powietrzu i odbudową w toku. Push o 3 w nocy
      // ma to mówić wprost, zamiast zostawiać człowieka z samym słowem DESTROY w nawiasie.
      if (attacks.length) {
        const k = `atak|${attacks.map(t => t.id || t.dst).join(",")}`;
        const wKsiezyc = attacks.some(t => /DESTRUCT|DESTROY/i.test(String(t.type || "")));
        if (!Once.said(k, 10 * 60e3)) Journal.add("ATAK", (wKsiezyc ? "CELEM JEST KSIĘŻYC (DESTROY) — uciekam „stacjonuj” najwolniej jak fork pozwala i zawrócę flotę, gdy minie; jeśli księżyc padnie, odbuduję go. " : "") + attacks.map(t => `${t.type} → [${t.dst}] ${t.dstBody || "?"} za ${Math.round((t.arriveAt - Date.now()) / 1000)}s (${t.source})`).join("; "));
      }
      ...
      if (mNow && ECO_KIND(mNow.kind)) {
        ...
        // v3.68.5 (audyt 04.09, obrona-decide#3 + obrona-wykonanie#2, P1): wywłaszczenie
        // przy alarmie znało WYŁĄCZNIE trzy misje ekonomii, a misja obronno-rutynowa ma
        // `kind:"fly"` albo `"home"` (Fleet Save, „dom = księżyc", „powrót po ratunku",
        // ręczny powrót z panelu). Taka misja szła więc spokojnie dalej, a stojące niżej
        // `if (Fly.mission()) return` wycinało z przebiegu CAŁĄ listę akcji — nie tylko
        // ratunek, ale też `recall` i `extend`, czyli drugą połowę trwającej ucieczki.
        // Cisza obrony trwała do sufitu misji (5 min / 6 nawigacji). Lot DOBROWOLNY nie ma
        // prawa stać na drodze ratunku: przerywamy go, gdy gdziekolwiek leci atak albo
        // czeka gotowa akcja ratunkowa. Wyjątek jak przy wyłączaniu ekonomii: gdy wysyłka
        // JUŻ poszła (stempel `last_send` tej misji), przerwanie zgubiłoby wpis lotu —
        // wtedy normalna ścieżka musi ją najpierw zaksięgować.
        const urgent = (s.threats || []).some(t => t.attack && t.arriveAt > Date.now())
          || actions.some(a => (a.kind === "fly" && (a.rescue || a.blind)) || a.kind === "recall");
        const ls = Store.get("last_send", null);
        const wyslane = !!ls && ls.from === mNow.fromKey && ls.toKey === mNow.toKey && (ls.at || 0) >= (mNow.startedAt || 0);
        if (urgent && !wyslane) Fly.abort(`ALARM — ratunek ma pierwszeństwo przed lotem dobrowolnym (${mNow.why || mNow.kind})`, { quiet: true });
      }
      await Fly.tick();
      ...
      // v3.68.5 (audyt 04.09, obrona-decide#2/#4 + obrona-wykonanie#3 + testy-architektura#2):
      // ta łatka była zrobiona pod JEDNĄ flagę (`fs`) i o pół punktu — czyli nie łapała
      // ani lotu rutynowego („dom = księżyc", „powrót po ratunku" — kind:"fly", bez `fs`),
      // ani nie spychała FS za `recall` (1) i `extend` (2), czyli za DRUGĄ POŁOWĘ tej samej
      // operacji ratunkowej: FS zabierał slot misji, a zawrót uciekającej floty czekał do
      // końca misji FS i flota lądowała na obcym refugium. Odwracamy warunek z listy
      // wyjątków na listę uprawnionych: RANK 0 należy do RATUNKU (`rescue`/`blind`), a
      // każdy lot dobrowolny (dziś FS i lot domowy, jutro cokolwiek nowego) ląduje ZA całą
      // kolejką obrony. Drugi klucz to PILNOŚĆ z decide(): przy dwóch atakach naraz
      // ratujemy najpierw kolonię z krótszym dolotem, a przy remisie większy hangar —
      // wcześniej decydowała kolejność kolonii na pasku planet, czyli przypadek.
      const prio = (a) => (a.kind === "fly" && !a.rescue && !a.blind ? 5 : 0);
      actions.sort((x, y) => ((RANK[x.kind] ?? 9) + prio(x)) - ((RANK[y.kind] ?? 9) + prio(y))
        || ((x.etaMs ?? Infinity) - (y.etaMs ?? Infinity))
        || ((y.saveTotal ?? 0) - (x.saveTotal ?? 0)));
      const hasRescue = actions.some(a => (a.kind === "fly" && (a.rescue || a.blind)) || a.kind === "recall");
      // v3.68.5 (audyt 04.09, obrona-decide#2 P0): dla lotu DOBROWOLNEGO sama obecność
      // gotowej akcji ratunku to za słabe kryterium — w oknie potwierdzania zagrożenia
      // (confirmMs) akcji ratunku jeszcze NIE MA, a atak już leci. Wtedy właśnie Fleet Save
      // albo lot domowy bywał JEDYNYM `fly` w przebiegu i zabierał slot misji na 2–3
      // przeładowania gry. Do wstrzymania lotu dobrowolnego wystarczy sam ALARM.
      ...
      for (const a of actions) {
        ...
        if (a.kind === "fly") {
          // v3.68.5 (audyt 04.09, obrona-decide#2 P0): SAM SORT NIE WYSTARCZY. Gdy ratunek
          // odpadnie wyżej (karencja `Fly.blocked` — dla lotu ratunkowego raptem 45 s, sufit
          // prób, cokolwiek), pętla schodzi niżej i mimo wszystko wypala lot dobrowolny,
          // oddając mu jedyny slot misji i gasząc obronę na czas jego trwania. Dopóki na
          // koncie jest para pod ostrzałem (albo lot do zawrócenia), lot dobrowolny czeka.
          if (alarmNow && !a.rescue && !a.blind) {
            if (!Once.said(`rut|${a.fromKey}>${a.toKey}`, 5 * 60e3)) log(`[LOT] WSTRZYMANY lot dobrowolny [${a.fromKey}]→[${a.toKey}] (${a.why}) — na koncie jest para pod ostrzałem, jedyny slot lotu należy do obrony.`, "warn");
            continue;
          }
          ...
          // teraz tę z krótszym dolotem). Do 3.68.4 para odłożona nie dostawała ŻADNEGO
          // własnego sygnału: właściciel nie miał jak się dowiedzieć, że ma ją rozegrać
          // ręcznie. Push idzie tylko wtedy, gdy naprawdę coś odkładamy.
          if (a.rescue || a.blind) {
  ...
  const UI = {
    ...
    build() {
      ...
      $("ogx3-save").onclick = async () => {
        ...
        // v3.68.5 (audyt 04.09, obrona-decide#2): `!x.fs` odrzucało tylko Fleet Save, więc
        // awaryjny drugi `find` mógł nadal wybrać lot RUTYNOWY z cudzej pary („powrót po
        // ratunku", „dom = księżyc") — operator klikał „RATUJ FLOTĘ TERAZ", a bot zwoził
        // flotę gdzie indziej. Wybieramy wyłącznie akcje RATUNKOWE.
        const act = actions.find(x => x.kind === "fly" && x.fromKey === a0.key && (x.rescue || x.blind))
          || actions.find(x => x.kind === "fly" && (x.rescue || x.blind));
    ...
    renderStatus() {
      ...
      { const rh = `${String(CFG.fs.returnHour ?? 7).padStart(2, "0")}:${String(CFG.fs.returnMinute || 0).padStart(2, "0")}`;
        ...
        // v3.68.10 (audyt 04.09, obrona-fs#4 P1): wiersz FS świecił na zielono „w domu ·
        // wraca o 07:00" także wtedy, gdy Fleet Save był fizycznie NIEWYKONALNY (lot krótszy
        // niż połowa czasu do powrotu — patrz Fly, „za krótki") i bot całą dobę spalał na to
        // po trzy porzucone misje na godzinę. Operator nie miał jak tego zobaczyć. Odkąd
        // zmierzony czas lotu wraca do stanu (`s.fsMeasured`), pasek mówi, KIEDY FS wystartuje.
        const fsWait = !fsFlight && CFG.fs.enabled
          ? Object.values(s.fsMeasured || {}).map(v => ({ v, openAt: (s.fsReturnAt || 0) - 2 * ((v && v.flightMs) || 0) }))
  ...
  // Po każdej dłuższej ciszy wypisujemy z niego podsumowanie na samej górze logu.
  {
    ...
    if (ostatni && przerwa > 3 * 3600e3) {
      ...
      if (j.length) {
        // v3.77.0: „EKO" doliczone osobno — po wydzieleniu rodzaju zatrzymana ekspedycja
        // zniknęłaby z bilansu po przerwie, a to jest właśnie moment, w którym warto wiedzieć,
        // że przez noc nic nie latało. Rozwinięcie niżej zostaje przy obronie (ATAK/BŁĄD).
