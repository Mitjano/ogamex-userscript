...
| plik | uni | stan | uwaga |
|---|---|---|---|
| `ogamex-3.user.js` | **genesis.ogamex.net** | **AKTYWNY ROZWÓJ** (v3.104.0, ~6,7k linii) | tu idzie cała nowa praca; profil gracza: ODKRYWCA; gra chodzi w **Chrome** (od 09.09) |
...
- **Rozmiar fali ekspedycji (v3.103.x = v3.101.1 + poprawka formularza + tolerancja 1% na „cały hangar"):** udział jednej fali = `(hangar + flota w powietrzu z tej bazy) / min(fale, sloty expo)`. NIE `hangar / wolne sloty` i NIE „ostatni wolny slot bierze wszystko" — to zlepiało fale na stałe (v3.100.0) i 21.09 symulator obalił też każdą „wąską" postać tej reguły (HANDOFF 20b). Fala „cały hangar" bierze ilości z FORMULARZA (v3.99.2), ale `Fly` liczy udział jeszcze raz na tym, co formularz pokazuje (`shareCtx`): ≥ 1,5 udziału = od planu wylądowała druga fala → leci JEDEN udział (log 21.09 11:11: plan 2,8 mld, poleciało 5,6 mld, slot pusty ponad godzinę). Niepełny rejestr powrotów skalujemy liczbą lotów z GRY (`expo.used`), nigdy licznikiem serii. Znany koszt wzoru: ~0,2% floty stoi w domu przy pełnych slotach — świadomy kompromis. Przy zaległości ≥ 2 udziałów w stanie ustalonym odstęp między falami nie obowiązuje (v3.104.0). **Flota stojąca w domu przy pełnych slotach w ilości ~fali = najpewniej rejestr powrotów zgubił falę** (lądowanie w sekundzie wysyłki policzone jako „gra wysłała mniej") — bot loguje to jako spadek bilansu floty; HANDOFF 20e. Historia 21.09 (wycofanie do 3.100 jako v3.102.0 i powrót): HANDOFF sekcja 20–20c. **Dowodem w ekonomii jest log z żywej gry** — `tools/win_expo_report.py` (snap/report), dane poza repo w `~/ogamex-expo-logs`.
- **Fork NIE wysyła mniej, niż wpisano** (dowód 22.09 12:04, HANDOFF 20f): „w hangarze zostało ponad plan" po wysyłce = fala wylądowała w sekundzie kliknięcia. Rejestr powrotów trzyma `last_send.total` (pola formularza), nigdy wynik odejmowania hangaru — to zaniżało flotę w powietrzu o całą falę.
- **Opis lotu (`m.why`) bywa KONTRAKTEM między modułami, nie tylko tekstem do logu.** `Fly` rozpoznaje po frazie „cały hangar" falę, po której odprowadza operatora na jego stronę. Przed zmianą tekstu w `expoPlan`: `grep` po frazie (E2E sc. 36).
