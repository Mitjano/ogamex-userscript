...
- **Rozmiar fali ekspedycji (v3.103.x = v3.101.1 + poprawka formularza + tolerancja 1% na „cały hangar"):** udział jednej fali = `(hangar + flota w powietrzu z tej bazy) / min(fale, sloty expo)`. **Fala na OSTATNI wolny slot wg gry bierze CAŁY hangar bez progu (v3.105.0, DECYZJA OWNERA 22.09: pełne sloty i pusty hangar > równość fal; symulator pokazuje koszt ~6% fal i rozrzut 2–27×, owner go zna — nie cofać bez jego zgody).** Przy ≥ 2 wolnych slotach: udział z całej floty, NIE `hangar / wolne sloty` (to zlepiało fale w v3.100.0). Fala „cały hangar" bierze ilości z FORMULARZA (v3.99.2), ale `Fly` liczy udział jeszcze raz na tym, co formularz pokazuje (`shareCtx`): ≥ 1,5 udziału = od planu wylądowała druga fala → leci JEDEN udział (log 21.09 11:11: plan 2,8 mld, poleciało 5,6 mld, slot pusty ponad godzinę). Niepełny rejestr powrotów skalujemy liczbą lotów z GRY (`expo.used`), nigdy licznikiem serii. Znany koszt wzoru: ~0,2% floty stoi w domu przy pełnych slotach — świadomy kompromis. Przy zaległości ≥ 2 udziałów w stanie ustalonym odstęp między falami nie obowiązuje (v3.104.0). **Flota stojąca w domu przy pełnych slotach w ilości ~fali = najpewniej rejestr powrotów zgubił falę** (lądowanie w sekundzie wysyłki policzone jako „gra wysłała mniej") — bot loguje to jako spadek bilansu floty; HANDOFF 20e. Historia 21.09 (wycofanie do 3.100 jako v3.102.0 i powrót): HANDOFF sekcja 20–20c. **Dowodem w ekonomii jest log z żywej gry** — `tools/win_expo_report.py` (snap/report), dane poza repo w `~/ogamex-expo-logs`.
