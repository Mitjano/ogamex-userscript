...
check("16:22: pasek 1, 2 sondy WYLĄDOWANE → nadwyżka 1, czekanie 30 s od KANDYDATA", r.excess === 1 && r.waitUntil === T + 30000);
let r2 = decide({ ...base, barForeign: 1, attacks: 0, spies: 2, spiesInFlight: 0, spyMaxEta: 0, now: T + 30000 });
check("16:22 po 30 s: czekanie NIE przesuwa się z odczytem (kotwica = kandydat) → już minęło", r2.waitUntil === T + 30000 && r2.now === undefined && (T + 30000) >= r2.waitUntil);
...
check("brak kandydata → kotwica = now", r.waitUntil === T + 30000);
...
check("16:33:09: 3 wylądowane(<30 s) sondy, eta 19 → czekanie = lądowanie + 30 s", r.excess === 3 && r.waitUntil === T + 49000);
...
check("eta 0 bez zapamiętanego lądowania → okno NIE przesuwa się z now (kotwica+30 s, już minęło)", r.waitUntil === T + 30000);
...
check("26.08 16:03: alarm trwa, atak(księżyc)+wylądowana sonda, pasek 2 → nadwyżka 1 czeka 30 s od POCZĄTKU NADWYŻKI (nie od firstAt)", r.excess === 1 && r.waitUntil === ONSET + 30000 && r.why === "nadwyżka może być sondą");
r = decide({ ...base, barForeign: 2, attacks: 1, spies: 1, spiesInFlight: 0, spyMaxEta: 0, candidateAt: FIRST, excessSince: 0, now: ONSET });
check("bez excessSince (stary kod) czekanie już minęło → to był błąd 26.08", r.waitUntil === FIRST + 30000 && ONSET >= r.waitUntil);
r = decide({ ...base, barForeign: 1, attacks: 0, spies: 2, spiesInFlight: 0, spyMaxEta: 0, candidateAt: T + 5000, excessSince: T, now: T + 5000 });
check("nadwyżka starsza niż kandydat (16:22) → kotwica = kandydat (nie cofa się)", r.waitUntil === T + 5000 + 30000);
