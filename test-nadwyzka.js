// ─────────────────────────────────────────────────────────────────────────
//  TEST NADWYŻKI PASKA — barExcessDecision  (v2.104.0)
// ─────────────────────────────────────────────────────────────────────────
// Wykonuje PRAWDZIWĄ funkcję ThreatMonitor.barExcessDecision wyciętą z bota
// (blok BAR-EXCESS) na danych z incydentów. Powód: 2.103.3 kotwiczyła
// czekanie „na lądowanie sond" w chwili odczytu (odnawianej co tick) — alarm
// z wylądowanymi sondami nigdy się nie potwierdzał; audyt 25.08 wieczorem.
const fs = require("fs"), path = require("path");
const src = fs.readFileSync(path.join(__dirname, "ogamex-bot.user.js"), "utf8").replace(/\r\n/g, "\n");
const a = src.indexOf("    barExcessDecision({"), b = src.indexOf("    // ── /BAR-EXCESS ──");
if (a < 0 || b < 0) throw new Error("brak bloku BAR-EXCESS");
const fnSrc = src.slice(a, b).trim().replace(/^barExcessDecision/, "function barExcessDecision").replace(/,\s*$/, "");
const decide = new Function(fnSrc + "\nreturn barExcessDecision;")();

let failures = 0;
const check = (n, c) => { console.log(`${c ? "OK  " : "FAIL"} | ${n}`); if (!c) failures++; };
const T = 1_700_000_000_000; // kandydat
const base = { barCached: false, barCountsProbes: false, candidateAt: T, now: T };

let r = decide({ ...base, barForeign: 3, attacks: 0, spies: 3, spiesInFlight: 3, spyMaxEta: 60 });
check("21:34: pasek 3, 3 sondy W LOCIE → nadwyżka 0 (BEZ alarmu)", r.excess === 0 && r.waitUntil === 0);

r = decide({ ...base, barForeign: 1, attacks: 0, spies: 2, spiesInFlight: 0, spyMaxEta: 0 });
check("16:22: pasek 1, 2 sondy WYLĄDOWANE → nadwyżka 1, czekanie 60 s od KANDYDATA (trwałość)", r.excess === 1 && r.waitUntil === T + 60000);
let r2 = decide({ ...base, barForeign: 1, attacks: 0, spies: 2, spiesInFlight: 0, spyMaxEta: 0, now: T + 60000 });
check("16:22 po 30 s: czekanie NIE przesuwa się z odczytem (kotwica = kandydat) → już minęło", r2.waitUntil === T + 60000 && r2.now === undefined && (T + 60000) >= r2.waitUntil);

r = decide({ ...base, barForeign: 1, attacks: 0, spies: 0, spiesInFlight: 0, spyMaxEta: 0 });
check("13:10: pasek 1, lista 0, bez sond → nadwyżka 1, bez czekania (ALARM)", r.excess === 1 && r.waitUntil === 0);

r = decide({ ...base, barForeign: 2, attacks: 0, spies: 1, spiesInFlight: 1, spyMaxEta: 1800 });
check("atak z układu + DALEKA sonda w locie (eta 30 min) → nadwyżka 1, sondy na liście → trwałość, cap kotwica+150 s", r.excess === 1 && r.waitUntil === T + 150000);

r = decide({ ...base, barForeign: 2, attacks: 0, spies: 1, spiesInFlight: 0, spyMaxEta: 1800 });
check("v2.104.4: nadwyżka 2 > 1 wylądowana sonda → LICZBY nie rozstrzygają, trwałość (cap 150 s)", r.excess === 2 && r.waitUntil === T + 150000 && /trwałość/.test(r.why));

r = decide({ ...base, barForeign: 1, attacks: 0, spies: 1, spiesInFlight: 0, spyMaxEta: 1800 });
check("cap: wylądowana sonda z eta 1800 (śmieć) → czekanie ≤ kandydat + 150 s", r.excess === 1 && r.waitUntil === T + 150000);

r = decide({ ...base, barForeign: 3, attacks: 0, spies: 3, spiesInFlight: 0, spyMaxEta: 0, barCached: true });
check("21:34:33 cache paska + sondy wylądowały → nadwyżka ZOSTAJE, krótkie czekanie 20 s na żywy pasek", r.excess === 3 && r.waitUntil === T + 20000);

r = decide({ ...base, barForeign: 2, attacks: 1, spies: 1, spiesInFlight: 1, spyMaxEta: 40 });
check("ACS na liście + sonda w locie: pasek 2 = 1+1 → nadwyżka 0 (atak i tak z listy)", r.excess === 0);

r = decide({ ...base, barForeign: 3, attacks: 0, spies: 3, spiesInFlight: 3, spyMaxEta: 60, barCountsProbes: true });
check("barCountsProbes=true: wszystkie sondy odejmowane → 0", r.excess === 0);

r = decide({ ...base, barForeign: 1, attacks: 0, spies: 2, spiesInFlight: 0, spyMaxEta: 0, candidateAt: 0 });
check("brak kandydata → kotwica = now", r.waitUntil === T + 60000);

// v2.104.3 — 26.08 16:33: 3 sondy (eta 19 s), pasek 3, lista 0 ataków; 20 s później
// sondy wylądowały (eta 0), pasek 2. Stare okno = kotwica+eta(teraz)+10 s → już minęło
// 1 s po lądowaniu. Nowe: lądowanie + 30 s karencji.
r = decide({ ...base, barForeign: 3, attacks: 0, spies: 3, spiesInFlight: 0, spyMaxEta: 19, candidateAt: T, excessSince: T, now: T });
check("16:33:09: 3 sondy eta 19 → czekanie = max(kotwica+60 s, lądowanie+30 s) = 60 s", r.excess === 3 && r.waitUntil === T + 60000 && r.landAt === T + 19000);
r = decide({ ...base, barForeign: 2, attacks: 0, spies: 3, spiesInFlight: 0, spyMaxEta: 0, candidateAt: T, excessSince: T, landAt: T + 19000, now: T + 20000 });
check("16:33:29: sondy wylądowały, pasek jeszcze 2 → czekanie NIE skurczyło się poniżej lądowanie+30 s", r.excess === 2 && r.waitUntil >= T + 49000 && (T + 20000) < r.waitUntil);
r = decide({ ...base, barForeign: 1, attacks: 0, spies: 2, spiesInFlight: 0, spyMaxEta: 0, candidateAt: T, excessSince: T, landAt: 0, now: T + 60000 });
check("eta 0 bez zapamiętanego lądowania → okno NIE przesuwa się z now (kotwica+60 s)", r.waitUntil === T + 60000);
// v2.104.4 — 26.08 16:45:49: pasek 4, lista 0 ataków + 2 sondy (eta 0). Stary kod: 4 > 2 → bez
// czekania → alarm po 12 s → ucieczka. Pasek był pusty 24 s później.
r = decide({ ...base, barForeign: 4, attacks: 0, spies: 2, spiesInFlight: 0, spyMaxEta: 0, candidateAt: T, excessSince: T, now: T });
check("16:45: pasek 4 > 2 sondy → i tak czeka 60 s na trwałość", r.excess === 4 && r.waitUntil === T + 60000);
r = decide({ ...base, barForeign: 4, attacks: 0, spies: 2, spiesInFlight: 0, spyMaxEta: 0, candidateAt: T, excessSince: T, now: T + 20000 });
check("16:46:09 (20 s później): jeszcze w oknie → BEZ alarmu", (T + 20000) < r.waitUntil);
check("parseBar: „No fleet movement” = żywy pasek z zerem (nie null → nie cache)", /if \(!m\) return \/No fleet movement\/i\.test\(t\) \? \{ total: 0, own: 0, foreign: 0 \} : null;/.test(src));
check("check() pamięta lądowanie (KEY_EXCESS_LAND, max) i kasuje z nadwyżką / clear()",
  /landAt: excLand/.test(src) && /if \(bx\.excess > 0 && \(bx\.landAt \|\| 0\) > excLand\)/.test(src) && /GM_setValue\(this\.KEY_EXCESS_LAND, "0"\); \}/.test(src));

// v2.104.2 — 26.08 16:03:18 / 16:06:03: alarm trwa (firstAt = 16:02:26), atak na
// księżyc na liście, wylądowana sonda też, pasek 2 vs lista 1. Kotwicą MUSI być
// początek tej nadwyżki, nie firstAt alarmu — inaczej „oba ciała" i 4 h lotu.
const FIRST = T, ONSET = T + 90000;
r = decide({ ...base, barForeign: 2, attacks: 1, spies: 1, spiesInFlight: 0, spyMaxEta: 0, candidateAt: FIRST, excessSince: ONSET, now: ONSET });
check("26.08 16:03: alarm trwa, atak(księżyc)+wylądowana sonda, pasek 2 → nadwyżka 1 czeka 60 s od POCZĄTKU NADWYŻKI (nie od firstAt)", r.excess === 1 && r.waitUntil === ONSET + 60000 && r.why === "nadwyżka może być sondą");
r = decide({ ...base, barForeign: 2, attacks: 1, spies: 1, spiesInFlight: 0, spyMaxEta: 0, candidateAt: FIRST, excessSince: 0, now: ONSET });
check("bez excessSince (stary kod) czekanie już minęło → to był błąd 26.08", r.waitUntil === FIRST + 60000 && ONSET >= r.waitUntil);
r = decide({ ...base, barForeign: 1, attacks: 0, spies: 2, spiesInFlight: 0, spyMaxEta: 0, candidateAt: T + 5000, excessSince: T, now: T + 5000 });
check("nadwyżka starsza niż kandydat (16:22) → kotwica = kandydat (nie cofa się)", r.waitUntil === T + 5000 + 60000);
check("check() śledzi początek nadwyżki (KEY_EXCESS_SINCE) i kasuje go przy zerze / clear()",
  /excessSince: excSince/.test(src) && /GM_setValue\(this\.KEY_EXCESS_SINCE, String\(excSince\)\); bx = decideBx\(\);/.test(src)
  && /else if \(bx\.excess <= 0 && excSince\) \{ excSince = 0; excLand = 0; GM_setValue\(this\.KEY_EXCESS_SINCE, "0"\); \}/.test(src)
  && /clear\(\) \{ GM_setValue\(this\.KEY, "null"\); GM_setValue\(this\.KEY_EXCESS_SINCE, "0"\); GM_setValue\(this\.KEY_EXCESS_LAND, "0"\); \}/.test(src));

check("check() używa barExcessDecision z kandydatem i wpisuje excess=0 w czasie czekania",
  /const decideBx = \(\) => barEff \? this\.barExcessDecision\(\{/.test(src) && /candidateAt: candAt/.test(src) && /excess: \(probeWaitUntil && Date\.now\(\) < probeWaitUntil\) \? 0 : barExcess/.test(src));
check("attackBodiesFor: klauzula „nadwyżka = oba ciała” tylko dla pary strzeżonej",
  /w\.armed && RescueQueue\.str\(w\.at\) === key\) \{ out\.add\("moon"\); out\.add\("planet"\); \}/.test(src));

console.log(failures ? `\n${failures} FAIL` : "\nWSZYSTKO OK");
process.exit(failures ? 1 : 0);

// v2.104.6 — 26.08 17:50: pasek „1 Hostile, Type: Spy”, lista bez wiersza sondy
{
  const r = barExcessDecision({ barForeign: 1, barSpyOnly: true, attacks: 0, spies: 0, spiesInFlight: 0, spyMaxEta: 0, candidateAt: 0, excessSince: 0, landAt: 0, now: 1000000 });
  must("pasek Type:Spy przy 1 obcym = brak nadwyżki", r.excess === 0);
  const r2 = barExcessDecision({ barForeign: 2, barSpyOnly: false, attacks: 0, spies: 0, spiesInFlight: 0, spyMaxEta: 0, candidateAt: 0, excessSince: 0, landAt: 0, now: 1000000 });
  must("2 obcych bez typu = nadwyżka jak dotąd", r2.excess === 2);
}

// v2.105.0 — 18:14: pasek „4 Hostile, Type: Spy” (rój sond), lista bez ataków
{
  const r = barExcessDecision({ barForeign: 4, barSpyType: true, attacks: 0, spies: 0, spiesInFlight: 0, spyMaxEta: 0, candidateAt: 1000000, excessSince: 1000000, landAt: 0, now: 1000000 });
  must("rój sond (Type Spy, lista bez ataków): brak nadwyżki, flota stoi", r.excess === 0);
  const r2 = barExcessDecision({ barForeign: 4, barSpyType: true, attacks: 1, spies: 0, spiesInFlight: 0, spyMaxEta: 0, candidateAt: 1000000, excessSince: 1000000, landAt: 0, now: 1000000 });
  must("Type Spy + atak na liście = nadwyżka liczona jak dotąd", r2.excess === 3);
}
